...
def start_device_persistent_monitoring(device_id: str,when_true:str,then_do:str):
    """
    持续监控：当某个设备的某些状态满足条件时，执行某些操作
    :param device_id:
    :param when_true: 自然语言描述，当某个设备的状态处于某些条件时
    ...
    prompt = f"""
            ...
            1. 一个设备有多个实体，你可以首先调用工具查看设备下所有实体各自能得到什么状态属性（比如亮度、温度、音量等等），以便确定哪个实体能完成任务
            2. 选择实体后再调用工具获取该实体的json数据，观察其结构组成,分析应该使用哪些字段来作为条件判断的依据。
            3. 编写代码，在代码中可以直接调用函数tool_get_states_by_entity_id()，系统会确保其在运行时存在
